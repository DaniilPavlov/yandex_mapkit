...
class MapObjectUpdates<T extends MapObject> extends Equatable {
  MapObjectUpdates.from(this.previous, this.current) {
    ...
    _objectsToRemove =
        previousObjectIds.difference(currentObjectIds).map((MapObjectId id) => previousObjects[id]!).toSet();

    _objectsToAdd = currentObjectIds.difference(previousObjectIds).map((MapObjectId id) => currentObjects[id]!).toSet();
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'toChange': _objectsToChange
          .map(
            (MapObject el) => el._updateJson(previous.firstWhere((MapObject prevEl) => prevEl.mapId == el.mapId)),
          )
          .toList(),
